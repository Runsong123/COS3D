...
class OpenCLIPNetwork:
    def __init__(self, device):
        ...
        print("self.neg_embeds.shape",self.neg_embeds.shape)
    ...
    def get_relevancy(self, embed: torch.Tensor, positive_id: int) -> torch.Tensor:
        # embed: 32768x512
        phrases_embeds = torch.cat([self.pos_embeds, self.neg_embeds], dim=0) # n_pos+n_neg, 512
        p = phrases_embeds.to(embed.dtype)
        output = torch.mm(embed, p.T) # 32768, n_pos+n_neg
        ...
        sims = torch.stack((repeated_pos, negative_vals), dim=-1) # 32768, n_neg, 2
        softmax = torch.softmax(10 * sims, dim=-1) # 32768, n_neg, 2
    ...
    def get_max_across_3D(self, segment_feature):
        n_phrases = len(self.positives)
        n_phrases_sims = [None for _ in range(n_phrases)]
        
        n_levels, segment_number, _ = segment_feature.shape
        clip_output = segment_feature.permute(1, 0, 2) # h*w, n_levels, 512
        ...
        relev_map = torch.stack(n_levels_sims) #.view(n_levels, n_phrases, segment_number)
        return relev_map